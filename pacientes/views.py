...
def paciente_view(request, id):
    ...
    if request.method == "GET":
        ...
        quant_consultas = consultas.count()
        tuple_grafico = ([str(i.data) for i in consultas], [str(i.humor) for i in consultas])
        return render(request, 'paciente.html', {'paciente': paciente, 'tarefas': tarefas, 'consultas': consultas,'quant_consultas': quant_consultas, 'tuple_grafico': tuple_grafico})
...
def consulta_publica(request, id):
    ...
    return render(request, 'consulta_publica.html', {'consulta': consulta, 'views': consulta.views})
